...
async function validateProject(projectPath, options) {
    logger_1.logger.step(`Validating project at ${chalk_1.default.cyan(projectPath)}`);
    const result = {
        valid: true,
        errors: [],
        warnings: [],
...
async function validateTemplate(templatePath, options) {
    logger_1.logger.step(`Validating template at ${chalk_1.default.cyan(templatePath)}`);
    const result = {
        valid: true,
        errors: [],
        warnings: [],
...
function displayValidationResult(result) {
    ...
    if (result.errors.length > 0) {
        ...
        result.errors.forEach(error => {
            logger_1.logger.plain(`  ${chalk_1.default.red('✗')} ${error}`);
        });
    }
    if (result.warnings.length > 0) {
        logger_1.logger.plain(`\n${chalk_1.default.yellow.bold('Warnings:')}`);
        result.warnings.forEach(warning => {
            logger_1.logger.plain(`  ${chalk_1.default.yellow('⚠')} ${warning}`);
        });
    }
    if (result.suggestions.length > 0) {
        logger_1.logger.plain(`\n${chalk_1.default.blue.bold('Suggestions:')}`);
        result.suggestions.forEach(suggestion => {
            logger_1.logger.plain(`  ${chalk_1.default.blue('💡')} ${suggestion}`);
        });
    }
    logger_1.logger.plain('');
}
