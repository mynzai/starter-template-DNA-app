...
function displayError(error) {
    ...
    let errorContent = `${chalk_1.default.red.bold('Error:')} ${message}`;
    if (code) {
        errorContent += `\n${chalk_1.default.gray(`Code: ${code}`)}`;
    }
    if (suggestion) {
        errorContent += `\n\n${chalk_1.default.yellow.bold('Suggestion:')} ${suggestion}`;
    }
    // Add common troubleshooting for specific error types
    const troubleshooting = getTroubleshootingHelp(code);
    if (troubleshooting) {
        errorContent += `\n\n${chalk_1.default.blue.bold('Troubleshooting:')}\n${troubleshooting}`;
    }
    // Add debug mode suggestion if not already in debug mode
    if (!environment_1.environment.debug) {
        errorContent += `\n\n${chalk_1.default.gray('For more details, run with --debug flag')}`;
    }
    console.error((0, boxen_1.default)(errorContent, {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'red',
...
function getTroubleshootingHelp(code) {
    const troubleshooting = {
        'TEMPLATE_NOT_FOUND': `• Check available templates with: ${chalk_1.default.cyan('dna-cli list')}\n• Verify template name spelling\n• Update template registry: ${chalk_1.default.cyan('dna-cli update')}`,
        'INVALID_PROJECT_NAME': `• Use alphanumeric characters, hyphens, and underscores only\n• Start with a letter\n• Keep length between 3-50 characters`,
        'DIRECTORY_EXISTS': `• Choose a different project name\n• Remove existing directory\n• Use ${chalk_1.default.cyan('--overwrite')} flag to replace existing files`,
