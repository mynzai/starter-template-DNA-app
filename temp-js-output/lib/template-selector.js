...
class TemplateSelector {
    ...
    async selectTemplate(options = {}) {
        ...
        if (allTemplates.length === 0) {
            console.log(chalk_1.default.yellow('⚠️  No templates found. Please check your template registry.'));
            return null;
        }
    ...
    async selectByCategory() {
        ...
        if (categoryNames.length === 0) {
            console.log(chalk_1.default.yellow('No template categories found.'));
            return null;
        }
    ...
    async searchTemplates() {
        ...
        if (results.length === 0) {
            console.log(chalk_1.default.yellow(`No templates found matching "${query}".`));
            // Offer to browse all templates
            const { browseFallback } = await inquirer_1.default.prompt([
                {
                    type: 'confirm',
                    name: 'browseFallback',
        ...
        console.log(chalk_1.default.green(`Found ${results.length} template(s) matching "${query}"`));
    ...
    async filterTemplates() {
        ...
        if (filtered.length === 0) {
            console.log(chalk_1.default.yellow('No templates match your filters.'));
            return null;
        }
        console.log(chalk_1.default.green(`Found ${filtered.length} template(s) matching your filters`));
    ...
    async collectFilters() {
        ...
        // Complexity filter
        const { selectedComplexity } = await inquirer_1.default.prompt([
            {
                type: 'list',
        ...
        // Setup time filter
        const { maxSetupTime } = await inquirer_1.default.prompt([
            {
                type: 'list',
                name: 'maxSetupTime',
    ...
    async selectRecommended() {
        ...
        if (recommended.length === 0) {
            console.log(chalk_1.default.yellow('No recommended templates available.'));
            return this.browseTemplates(20);
        }
        console.log(chalk_1.default.green('✨ Recommended templates (highest rated):'));
    ...
    formatTemplateChoice(template) {
        ...
        return `${chalk_1.default.cyan(template.name)} ${chalk_1.default.gray(`(${template.framework})`)} ${complexity} ${setupTime} ${rating}\n   ${chalk_1.default.dim(template.description)}`;
    }
    displayTemplateDetails(template) {
        console.log(chalk_1.default.bold(`📦 ${template.name}`));
        console.log(`   ${chalk_1.default.dim(template.description)}`);
        console.log('');
        console.log(chalk_1.default.bold('Details:'));
        console.log(`   Framework: ${chalk_1.default.cyan(template.framework)}`);
        ...
        if (template.dnaModules.length > 0) {
            console.log(chalk_1.default.bold('DNA Modules:'));
        ...
        if (template.features.length > 0) {
            ...
            if (template.features.length > 5) {
                console.log(`   ${chalk_1.default.dim(`   ... and ${template.features.length - 5} more`)}`);
            }
            console.log('');
        }
        if (template.tags.length > 0) {
            console.log(chalk_1.default.bold('Tags:'));
            console.log(`   ${template.tags.map(tag => chalk_1.default.blue(`#${tag}`)).join(' ')}`);
            console.log('');
        }
    }
        ...
        for (const variable of template.variables) {
            if (variable.required && !variables[variable.name]) {
                ...
                if (!variable.sensitive) {
                    const { [variable.name]: value } = await inquirer_1.default.prompt([prompt]);
                    variables[variable.name] = value?.toString() || '';
                }
                else {
                    // For sensitive variables, allow skipping with a note
                    const { shouldSet } = await inquirer_1.default.prompt([
                        {
                            type: 'confirm',
                            name: 'shouldSet',
                    ...
                    if (shouldSet) {
                        const { [variable.name]: value } = await inquirer_1.default.prompt([
                            {
                                ...prompt,
                                type: 'password',
                                mask: '*',
